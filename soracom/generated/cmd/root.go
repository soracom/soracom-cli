...
func init() {
	...
	RootCmd.PersistentFlags().StringVar(&providedAuthKeyID, "auth-key-id", "", "Specify AuthKeyId to be used for authentication. If both --auth-key-id and --auth-key are specified, soracom-cli works without profiles. It means you do not need to run 'soracom configure' before running the command. This may be suitable for temporary execution. Please do not forget specifying these options along with --coverage-type.")
	RootCmd.PersistentFlags().StringVar(&providedAuthKey, "auth-key", "", "Specify AuthKey to be used for authentication. If both --auth-key-id and --auth-key are specified, soracom-cli works without profiles. It means you do not need to run 'soracom configure' before running the command. This may be suitable for temporary execution. Please do not forget specifying these options along with --coverage-type.")
	RootCmd.PersistentFlags().StringVar(&providedProfileCommand, "profile-command", "", "Specify a command to provide a profile. If this option is specified, soracom-cli works without pre-configured profile. It means you do not need to run 'soracom configure' before running the command. This may be suitable for temporary execution. Please do not forget specifying this option along with --coverage-type.")
